...
public class Sorting {
    ...
    private static void quickSort3(int[] x, int low, int high) { 
       ...
       if(i<=j) {
    	   //get the left and right pivots from the array.
    	   int lp = x[i],rp = x[j];
    	   int l = i+1,g = j-1,k = l;
    	   
    	   while(k<=g) {
    		   //If the current value is less than the left pivot,
    		   //put it on the left side.
    		   if(x[k]<lp) {
    			   swap(x,k,l);
    			   l++;
    		   }
    		   //If the current value is greater than the right pivot,
    		   //put it on the right side.
    		   //Likewise, if the current value is less than the left pivot,
    		   //put it on the left side.
    		   else if(x[k]>=rp) {
    			   while(x[g]>rp && k<g) {
    				   g--;
    	   ...
    	   //Put each pivots to their positions
    	   swap(x,i,l);
    	   swap(x,j,g);
    	   
    	   //Recursion
    	   quickSort3(x,i,l-1);
    	   quickSort3(x,l+1,g-1);
    	   quickSort3(x,g+1,j);
       }
    }
